 ...
 */
 (function($) {
	 ...
	 var methods = {
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "creditCard":
						...
						break;
					case "ccExpiration":
						errorMsg = methods._ccExpiration(field, rules, i, options);
						break;
					case "usPostalCode":
						errorMsg = methods._usPostalCode(field, rules, i, options);
						break;
					case "caPostalCode":
						errorMsg = methods._caPostalCode(field, rules, i, options);
						break;
		...
		* @return an error string if validation failed
		*/
		_ccExpiration: function(field, rules, i, options) {		
			var year_field = $("#" + rules[i+1]);
			
			var d = new Date();
			var month = d.getMonth() + 1;
			var year = d.getFullYear().toString().slice(2);
			
			if(year >= parseInt(year_field.val(),10)) {
				if(month > parseInt(field.val(),10)) {
					return options.allrules[rules[i]].alertText;
				}
			}
		},
		...
		* @return an error string if validation failed
		*/
		_usPostalCode: function(field, rules, i, options) {					
			var str = field.val();
		...
		* @return an error string if validation failed
		*/
		_caPostalCode: function(field, rules, i, options) {					
			var str = field.val().toUpperCase();
			
			if(str.length) {
				if(!(/^([ABCEGHJKLMNPRSTVWXYZ]\d[ABCEGHJKLMNPRSTVWXYZ])( )?(\d[ABCEGHJKLMNPRSTVWXYZ]\d)$/.test(str))) {
					return options.allrules[rules[i]].alertText;
				}
			}
		},
